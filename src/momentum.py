...
def main():
    ...
    def apply_fig_settings(fig):
        ...
        if ("ylim" in item for item in documents["momentum"]):
    ...
    volume_drag_source.relim()
    volume_drag_source.autoscale()
    volume_drag_source.set_xlim(normalized_timespan)
    ...
    plt.savefig(meta_data.output_dir +
                "/cv_source_volume_drag.png", format='png')
    plt.show()
    ...
    tgp_decomp_b.legend(
        ["TGP force",
         ...
         r"$-(\frac{\rho c}{S_\mathrm{VT}}\langle{Q}_\mathrm{A}\rangle+\frac{\rho c}{S_\mathrm{VT}}\langle{Q}_\mathrm{D}\rangle)$"],
        bbox_to_anchor=(1.0, 0.5),
        loc='center left', ncol=1, labelspacing=2, frameon=False)
    # Save the plot
    plt.savefig(meta_data.output_dir +
